 ...
 * 
 * To save grayscale images, some values are skipped
 * with the side effect that old patterns are remained
 * So it's possible to put different patterns into the same image
 */
int cellWidth = 1;
int cellHeight = 1;
...
void keyPressed() {
  switch (key) {
    ...
    case '3':
      ...
      break;
    case 's':
        String s = String.format("your_output/c_bitwise_5a_%s_a_%d_s%s.png", bitwiseString, a, stamp());  // Information about the latest emerged pattern on screen. For distinction a stamp is added
        save(s);
