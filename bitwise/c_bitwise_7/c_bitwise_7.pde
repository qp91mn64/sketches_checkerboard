 ...
 * Created: 2026-02-23
 * 
 * Modefied from c_bitwise_6c
 * 
 * Bitwise operation, square, & a to extract bits, result used as color
 * Pretty complex patterns
 * 
 * Just drag to explore different parts
 * Colors are grayscale or ARGB
 * No fading in or accumulating effects
 * 
 * Left click: a + 1
 * Right click: a - 1
...
void keyPressed() {
  switch (key) {
    ...
    case '3':
      ...
      break;
    case 's':
        String s = String.format("your_output/c_bitwise_7_%s_a_%d_xOffset_%d_yOffset_%d_s%s.png", bitwiseString, a, xOffset, yOffset, stamp());  // For distinction a stamp is used
