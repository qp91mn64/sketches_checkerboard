 ...
 * 
 * Any color with nonzero alpha value is set opaque
 * 
...
PImage image1;
void setup() {
  size(1500, 700);  // See more patterns on the screen; make sure your computer support such high resolution
  noStroke();
...
void draw() {
  ...
  for (int x = 0; x < xMax; x++) {
    for (int y = 0; y < yMax; y++) {
      ...
      if (color1 >> 24 != 0) {  // Any color with nonzero alpha value is set opaque
        color1 |= 0xff000000;
      }
...
void keyPressed() {
  switch (key) {
    ...
    case '3':
      ...
      break;
    case 's':
        String s = String.format("your_output/c_bitwise_6c_%s_a_%d_xOffset_%d_yOffset_%d_s%s.png", bitwiseString, a, xOffset, yOffset, stamp());  // For distinction a stamp is used
