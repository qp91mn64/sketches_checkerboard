 ...
 * 
 * XOR Patterns with transparency:
 * When x < 4096 and y < 4096, b * b < 16777216, so that alpha is 0, no color
 * Except that grayscale pattern emerge on the main diagonal
 * When one of x >= 4096 (and not too large), colors fade in with low opacity
 * But when both x and y are in range [4096, 8192)
 * Grayscale pattern emerged again
 * 
 * Left click: a + 1
 * Right click: a - 1
 ...
 * 
 * Side effect: old patterns remain after switching
 * So it's possible to put different patterns into one image
 */
int cellWidth = 1;
int cellHeight = 1;
...
void keyPressed() {
  switch (key) {
    ...
    case '3':
      ...
      break;
    case 's':
        String s = String.format("your_output/c_bitwise_6_%s_a_%d_s%s.png", bitwiseString, a, stamp());  // For distinction a stamp is used
